...
In the current implementation of \texttt{pyJac}, we assume a constant-pressure system~\footnote{Note: in this context constant-pressure is used to signify a that the pressure is constant during the reaction sub-step of the operator-splitting scheme only (and is thus an input variable to the problem), not that the pressure is fixed throughout a reacting flow simulation.  This assumption is commonly used in multi-dimensional reacting flow simulations and is equivalent to that used in standard chemical-kinetic evaluation routines such as \texttt{CKYWP} in \texttt{CHEMKIN}~\cite{chemkin:2012}}\todo{why is there a space between the word and the footnote number?}; thus, we use Eq.~\eqref{e:state} to determine density rather than including it in the differential system~\eqref{e:vars}.
