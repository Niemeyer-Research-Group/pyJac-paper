...
\todo{Are we doing sparsity? We mention it in the abstract}
...
\mathcal{J}_{k+1,j+1} &\pluseq& \nu_{ki}\frac{W_k}{W_j} \Biggr[
    T_{\partial Y,i,\text{ind}} + T_{\partial Y, i, \text{falloff}} \; c_i\left(T_{P_{r,i}} + T_{F_i}\right)\left(\frac{\delta_{M}\left(\alpha_{ij} - \hat{\alpha}_{ij}\right)}{\left[X\right]_i} + \frac{1 - \delta_{\text{M}}}{\rho Y_m}\right)R_i \nonumber \\
&& +\: \left(
    c_i\nu_{ji}^{\prime}k_{f,i}\left[X_j\right]^{\nu_{ji}^{\prime}-1}
  \right)
  \prod_{\substack{n=1 \\ n \neq j}}^{N_{\text{sp}}}[X_n]^{\nu_{ni}^{\prime}} \nonumber \\
...
In order to demonstrate the correctness and computational performance of the generated analytical Jacobian matrices, multiple chemical kinetic models were chosen as test cases, selected to represent a wide spectrum of sizes, classes of fuel species, and types of reaction rate pressure dependence formulations (e.g., falloff, Chebyshev, pressure log).\todo{do we have plog / cheby?}
...
In reporting the discrepancy between the analytical and numerical Jacobian matrices, denoted $\mathcal{J}$ and $\hat{\mathcal{J}}$ respectively, two quantities are used: the relative error $p$-norm $E_\text{norm} = \norm{ \hat{\mathcal{J}} - \mathcal{J} }_p / \norm{\hat{\mathcal{J}}}_p$, where $p = 2$ (the Frobenius norm) and the maximum element relative error $E_\text{rel} = \max_{i, j} \abs{ ( \hat{\mathcal{J}}_{ij} - \mathcal{J}_{ij} ) / \hat{\mathcal{J}}_{ij} }$.
The former indicates overall error in the matrix but can be dominated by large elements, so the latter error measure can be useful in identifying discrepancies in the smaller matrix elements.
...
The CPU Jacobian was compiled using \texttt{gcc} 4.4.7, and run on two 6-core Xeon X5650 processors, parallelized using OpenMP.
