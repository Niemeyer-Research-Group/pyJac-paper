...
Table~\ref{t:cpu_comp} summarizes the performance ratio between Jacobian matrices calculated using \texttt{pyJac} and finite differences\slash \texttt{TChem}; \texttt{pyJac}-based routines perform approximately \SIrange{1.1}{2.2}{$\times$} faster than \texttt{TChem}, depending on the size of the model.
However, we again note that \texttt{TChem}'s lack of thread-safety implies that calculation of Jacobians for many different thermo-chemical states may be much faster using \texttt{pyJac} as the calculations may be accelerated easily using OpenMP.
