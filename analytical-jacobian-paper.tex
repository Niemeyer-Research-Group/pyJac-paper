...
First, we establish the correctness of the Jacobian matrices for kinetic models of hydrogen, methane, ethylene, and isopentanol oxidation (number of species ranging \SIrange{13}{360}) by showing agreement within \SI{1e-3}{\percent} of matrices obtained via automatic differentiation.
...
As the need for detailed and accurate chemical kinetics models~\footnote{Note that the term ``reaction mechanism'' is also used commonly in the literature; we adopted our preferred terminology ``chemical kinetic model'' here.} in predictive reactive-flow simulations has become recognized in recent years, simultaneously such models describing the oxidation of hydrocarbon fuels grew orders of magnitude in size and complexity.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Efficient evaluation of Jacobian matrix}
\label{section:eval}
...
{\allowdisplaybreaks \begin{IEEEeqnarray}{rCl}
\mathcal{J}_{k+1, j+1} & = & \frac{W_k}{\rho} \left( \dydx{\dot{\omega}_k}{Y_j} + \dot{\omega}_k W\left(\frac{1}{W_j} - \frac{1}{W_{\numsp}} \right) \right) \nonumber \\
...
Although others discuss sparsity of chemical kinetics Jacobian matrices (Lu and Law, Perini et al.~\cite{Perini:2012gy}); however, while many assume that large kinetic models exhibit a sparse Jacobian matrix, the Jacobian for the governing equations of constant-pressure kinetics is actually dense.
...
As a result, the partial derivatives of all species $k$ rate-of-change $\dot{Y}_k$ with respect to the $j$th species' mass fraction of species $Y_j$, or $\mathcal{J}_{k+1, j+1}$, are nonzero.
...
However, simulations of low-speed open combustion systems typically rely on the constant-pressure assumption~\cite{Law:2006tu}, with constant volume applicable to enclosed systems (e.g., internal combustion engine cylinder).
\todo[inline]{finish and revise discussion}

Alternatively, the governing equations could be formulated in terms of species molar concentrations rather than mass fractions~\cite{Lu:2009gh,Bisetti:2012jw}.
This would eliminate the presence of $\rho$ in the cross-species Jacobian matrix elements and thus significantly increase matrix sparsity for constant-pressure combustion.
...
\begin{equation}
  T_{\partial Y, i,\text{ind}} = -\frac{Wc_i}{\rho} \left( R_{f,i}\sum_{l=1}^{N_{sp}}\nu_{li}^{\prime} - R_{r,i}\sum_{l=1}^{N_{sp}}\nu_{li}^{\prime\prime}\right)
\end{equation}
...
\Biggl[ 
	T_{\partial Y,i,\text{ind}} \left(1 - \frac{W_j}{W_{\numsp}}\right) +
	c_i \left ( 
...
\end{IEEEeqnarray}}%
where $S_l^{\prime}$ and $S_l^{\prime\prime}$ are defined as:
{\allowdisplaybreaks \begin{IEEEeqnarray}{rCl}
S_l^{\prime} &=& \nu_{li}^{\prime} [X_l]^{\nu_{li}^{\prime} - 1} \prod_{\substack{n=1 \\ n \neq l}}^{\numsp}[X_n]^{\nu_{ni}^{\prime}} \nonumber \\
...
This means that the added $S_l^{\prime}$ and $S_l^{\prime\prime}$ terms are often zero, and may be omitted in many cases.
...
{\allowdisplaybreaks \begin{IEEEeqnarray}{rCl}
  T_{\partial Y, i, \text{third-body}} &=& 
  	...
  	&=& 
  		-\frac{Wc_i}{\rho} 
  		\left( 
  			R_{f,i}\left(1 + \sum_{l=1}^{N_{sp}}\nu_{li}^{\prime} \right) - 
  			R_{r,i}\left(1 + \sum_{l=1}^{N_{sp}}\nu_{li}^{\prime\prime}\right)
  		\right)
\end{IEEEeqnarray}}%
...
&& +\:  \frac{\frac{k_0}{k_{\infty}}F_i}{1 + Pr_i}\left(T_{P_{r,i}} + T_{F_i}\right)
		\left(
...
\todo{Get DOI for v1 release}
\texttt{pyJac} requires the Python module \texttt{NumPy}~\cite{Walt:2011aa}.
The module used to test the correctness of \texttt{pyJac} isincluded in this release, and additionally requires \texttt{Cython}~\cite{Behnel:2011aa}, Cantera~\cite{Goodwin:2015aa}, \texttt{PyYAML}~\cite{Simonov:2014aa}, and \texttt{Adept}~\cite{adept-v11}; these are not required for Jacobian/rate subroutine generation however.
...
USC-Mech II		& \num{900900}	& \SI{9.046e-9}{\percent} & \SI{2.356e-4}{\percent} \\
...
The CPU Jacobian was compiled using \texttt{gcc} 4.4.7, and run using two six-core Xeon X5650 processors.\todo{insert citation for gcc?}{}
Parallelization was achieved using OpenMP~\cite{Dagum:1998}, where each OpenMP thread was used to evaluate an entire Jacobian.
...
The GPU Jacobian was tested on a single NVIDIA Tesla C2075, and was compiled using \texttt{nvcc} 7.5\todo{insert citation for nvcc?}{}.
The mean GPU evaluation times were again computed from ten individual runs.
The finite difference routine for both the CPU and GPU was a simple first order forward difference adapted from CVODE~\cite{Hindmarsh:2005hg} to give a realistic comparison to a commonly used finite difference technique.
...
Figure~\ref{F:cpu_perf} displays the performance of the CPU-based \texttt{pyJac} Jacobian matrix evaluations for the four kinetic models; in addition, Fig.~\ref{F:cpu_perf} compares the performance of \texttt{pyJac} against that of the existing \texttt{TChem} package~\cite{Safta:2011vn}.
As summarized in Table~\ref{t:cpu_comp} \texttt{pyJac} is approximately \SIrange{4}{40}{$\times$} faster than \texttt{TChem} depending on the model in question.
\texttt{pyJac} is significantly faster than the first order finite difference technique as well, ranging from \SIrange{3}{6}{$\times$} faster depending on the model.

Additionally Fig.~\ref{F:cpu_perf} shows lines of best fit for the available data (the corresponding $R^2$ values were 0.86, 0.72, and 0.99 for the finite difference, \texttt{pyJac} and \texttt{TChem} respectively).
These fits suggest that all the available codes scale super-linearly with the number of species in the model.
This is expected for the finite difference technique, but merits some explanation for the analytical Jacobian methods.
...
However, this argument is predicated on the assumption that the resulting Jacobian was sparse, namely that only a change in species concentrations that participate in a reaction can change the resulting reaction rate.
This provides a significant incentive for development of a sparse concentration based constant pressure Jacobian system, and is a planned upgrade for \texttt{pyJac}.
This is not true in the constant pressure\slash mass-fraction based system due to the dependence of density on mass fraction as discussed previously (Sec.~\ref{S:sparsity}), hence we expect the execution times exhibit a higher order dependence on number of species (or reactions) in a model.
If we use the developed fits to get a rough order of magnitude estimate of the crossover points of the various models we see that \texttt{pyJac} is expected to outperform \texttt{TChem} for models with up to $\sim$2500 reactions, and outperform the finite difference technique for models with over eighty-million reactions.
...
\begin{figure}[tbp]
    ...
    The dashed line indicates a linear scaling of evaluation time with the number of reactions in the model.}
    \label{F:cpu_perf}
\end{figure}
...
\begin{tabular}{@{}l l l@{}}
\toprule
Model & $\bar{R}_{\texttt{TChem}} / \bar{R}_{\texttt{pyJac}}$ & $\bar{R}_{\text{F.D.}} / \bar{R}_{\texttt{pyJac}}$ \\
\midrule
\ce{H2}\slash \ce{CO} & 40.95 & 5.28 \\
GRI-Mech 3.0 &  11.11 & 6.30 \\
USC-Mech II &  3.93 & 4.28 \\
\ce{iC5H11OH} & N\slash A & 2.89 \\
\bottomrule
\end{tabular}
\caption{Ratio of Jacobian matrix evaluation times for \texttt{TChem} and finite differencing with \texttt{pyJac}. 
...
It is noted that a `per-thread` evaluation model has been adopted in this work, in which a single GPU thread is responsible for evaluation of a single Jacobian matrix, rather than a `per-block` model where GPU threads in a block cooperate to evaluate a Jacobian.
This is advantageous in that it significantly simplifies generation of highly optimized code for SIMD processors and provides a higher theoretical bound on the number of Jacobians that can be evaluated in parallel.
However, this is a topic that merits further investigation; the current Jacobian implementation is memory bandwidth limited due to the small cache sizes available on the GPU's streaming microprocessors.
A per-block implementation would likely be able to utilize this small cache more effectively, but it remains to be seen if this will translate into an overall speedup.
...
As with the CPU matrix evaluations shown in Fig.~\ref{F:cpu_perf}, we observed a superlinear scaling of the performance with number of reactions.
The best fits for the \texttt{pyJac} and the finite difference technique (with $R^2$ values of 0.95 and 0.77 respectively) are of similar polynomial orders as those of the CPU-based Jacobian evaluation, providing evidence that this is close to the scaling of the method itself.

Table~\ref{t:gpu_comp} shows the ratios of average evaluation time between the finite difference and \texttt{pyJac} on the GPU.
Interestingly, the speedup for using \texttt{pyJac} over the finite-difference technique on the isopentanol model is significantly higher than those of the other models.
This is again due to the small cache size of the GPU, in this case the much larger model size (360 species compared to say 111 species for USC-Mech II) likely makes it difficult to store a majority of species concentrations in the cache, forcing more loads from global memory.
This implies that it is potentially significantly more efficient to use an analytical Jacobian formulation on the GPU than a finite difference formulation for larger models.
Additionally, a sparse Jacobian formulation would greatly benefit GPU Jacobian evaluation due to the greatly reduced memory traffic requirements (i.e. a greatly reduced number a global writes).
...
\begin{figure}[tbp]
    ...
    \begin{subfigure}{0.5\textwidth}
        ...
        \caption{Normalized GPU runtime versus the number of reactions in the model.}
        \label{F:gpu_norm}
    \end{subfigure}
...
\begin{tabular}{@{}l l@{}}
\toprule
Model & $\bar{R}_{\text{F.D.}} / \bar{R}_{\texttt{pyJac}}$ \\
\midrule
\ce{H2}\slash \ce{CO} & 1.80 \\
GRI-Mech 3.0 &  1.74 \\
USC-Mech II &  1.61 \\
\ce{iC5H11OH} & 3.19 \\
\bottomrule
\end{tabular}
\caption{Ratio of Jacobian matrix evaluation times for finite differencing with \texttt{pyJac} on the GPU. 
$\bar{R}$ indicates the mean evaluation time.}
\label{t:gpu_comp}
\end{table}

The results presented above raise a number of issues that warrant further study.
The super-linear scaling of \texttt{pyJac} matrix evaluation time with number of reactions in the kinetic models is expected due to the density of the Jacobian, but points to the large potential benefits of a sparse species concentration based Jacobian implementation.
However, while \texttt{pyJac} outperforms \texttt{TChem} in all cases, the performance ratio between them drops with increasing kinetic model size, indicating that the performance benefit of a hard-coded Jacobian matrix evaluation subroutine is less important with increasing model size.
Although it appears from these results that the performance of a hard-coded Jacobian subroutine can surpass that of an interface based Jacobian routine, the hard-coded method has additional difficulties due to the size and number of files created.
For example, the CPU Jacobian evaluation subroutine for USC-Mech II comprises over \num{360000} total lines spread across 22 files (not including the supporting files for, e.g., species and reaction rate evaluations)---attempting to incorporate the entire matrix evaluation in a single file resulted in the \texttt{gcc} 4.4.7 compiler crashing.\todo{update these numbers}{}
This not only caused longer compile times (on the order of ten minutes to an hour when using a 24 separate compiler instances), but could cause compiler crashes and inexplicable behavior, particularly for the less mature \texttt{nvcc} compiler.
While we expect a sparse Jacobian formulation would significantly alleviate these issues, it is worthwhile to directly compare the performance of hard-coded and interface based Jacobian subroutines in the future.
...
In addition, we thank James Sutherland and Mike Hansen for helpful discussions on appropriate formulation of state vectors.
