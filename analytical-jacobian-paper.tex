...
This may be defined with explicit reverse Arrhenius parameters and therefore uses the same expression as Eq.~\eqref{e:arrhenius}.\todo{revisit wording}
Otherwise, the reverse rate coefficient is related to the forward coefficient through the equilibrium constant via
...
In order to demonstrate the correctness and computational performance of the generated analytical Jacobian matrices, we chose three chemical kinetic models as test cases, selected to represent a wide spectrum of sizes, classes of fuel species, and types of reaction rate pressure dependence formulations (e.g., Lindemann\slash Troe\slash SRI falloff formulations).
...
As a result, the Jacobian evaluation must be split into several smaller subroutines, which each evaluate the contributions of a number of reactions.
...
\bottomrule
\end{tabular}
\caption{Mean Jacobian evaluation runtime comparison for \texttt{TChem} and \texttt{pyJac}}
\label{t:cpu_comp}
\end{table}
