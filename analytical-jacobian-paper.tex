...
In addition, interpreting Cantera-format models~\cite{Goodwin:2015aa} requires that module, although Chemkin-format models do not require any additional components.
...
\begin{tabular}{@{}l l@{}}
\toprule
Model & $\bar{R}_{\texttt{TChem}} / \bar{R}_{\texttt{pyJac}}$ \\
\midrule
...
However, while \texttt{pyJac} outperforms \texttt{TChem} by a factor of 2.8 for the \ce{H2}\slash\ce{CO} model, the performance ratio between them drops with increasing kinetic model size, with \texttt{pyJac} performing worse for USC-Mech II.
