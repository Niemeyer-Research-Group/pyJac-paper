...
Typically, these are calculated numerically using finite differences, but this poses significant computational demands for larger chemical kinetic models since the number of chemical source term evaluations scales with the square of species count.
...
As the need for detailed and accurate chemical kinetics models in predictive reactive-flow simulations has become recognized in recent years, simultaneously such models describing the oxidation of hydrocarbon fuels grew orders of magnitude in size and complexity.
...
As will be discussed in greater detail, unlike the current work, the approach of Dijkmans et al.~\cite{Dijkmans:2014bb} used the GPU to calculate the elements of a single Jacobian matrix in parallel, rather than a large number of different matrices corresponding to different states.
...
In addition, since the programming paradigm to achieve efficient code evaluation for SIMD processors differs from that of normal CPUs, optimization strategies specifically targeted at accelerating Jacobian evaluation on GPUs will be laid out and tested.
...
The bulk of the computation is performed once per reaction, and only minor sub-products must be computed for each species-species pair for a given reaction.\todo{revisit this paragraph}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Cache Optimization}
\label{s:cache_opt}

In order to further improve performance, \texttt{pyJac} offers the ability to reorder the species and reactions in a kinetic mechanism in order to improve cache hit rates.
This is especially important for SIMD devices such as GPUs, as they typically have small L1 caches and thus loads from global memory can present a significant computational bottleneck.
...
As a practical note, the size of the generated Jacobian subroutines are such that even modern compilers such as \texttt{gcc} 4.4.7 and the CUDA compiler \texttt{nvcc} 7.0 can crash when compiling the generated Jacobian for even modestly sized kinetic models (e.g., GRI~\cite{smith_gri-mech_30} for \texttt{nvcc} and USC Mech II~\cite{Wang:2007} for \texttt{gcc}).
\texttt{pyJac} avoids this limitation by splitting large Jacobian matrices into multiple smaller subroutines when appropriate, which are compiled separately and called from the main Jacobian subroutine.
When starting a new subfile, the initial seed reaction is selected from the set of remaining unordered reactions.
...
In order to further reduce costly global memory loads on SIMD devices, a shared memory caching strategy can be employed in combination with the cache optimization detailed above.
...
The reaction rate subroutine only reuses the species concentrations values between reactions.
...
First, SIMD devices can execute all species rate updates for a reaction with no chance of a stall due to data dependence as all the species rate updates are independent.
Second, the net reaction rate is used between multiple update steps and is thus highly likely to be found in the local L1 cache or in a register, cutting down global memory loads.
...
In order to demonstrate the correctness and computational performance of the generated analytical Jacobian matrices, multiple chemical kinetic models were chosen as test cases, selected to represent a wide spectrum of sizes, classes of fuel species, and types of reaction rate pressure dependence formulations (e.g., Lindemann\slash Troe\slash SRI falloff formulations). \todo{Add Chebyshev, pressure log when true}
...
Adjusting the relative and absolute tolerances reduced some of these errors, but consistent values could not be found that sufficed over all states considered---in particular, unreasonably small tolerances (e.g., \num{1e-20}) were required for states near equilibrium, but these same tolerances caused larger errors at other states.
...
In reporting the discrepancy between the analytical and numerical Jacobian matrices, denoted $\mathcal{J}$ and $\hat{\mathcal{J}}$ respectively, the  norm of the relative errors of matrix elements $E_\text{rel} = \norm{ ( \hat{\mathcal{J}}_{ij} - \mathcal{J}_{ij} ) / \hat{\mathcal{J}}_{ij} }_p$, where $p = 2$ (the Frobenius norm), was used to quantify error.
...
\label{s:performance}
In order to test the performance of \texttt{pyJac}, the PaSR thermochemical composition data generated for validation of each kinetic model was used.
