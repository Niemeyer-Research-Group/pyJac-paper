...
\texttt{pyJac} requires the Python modules \texttt{NumPy}~\cite{Walt:2011aa}; optional dependencies include \texttt{bitarray}~\cite{bitarray:2015} for the cache optimizer, and the performance testing module additionally requires \texttt{Cython}~\cite{Behnel:2011aa}, Cantera~\cite{Goodwin:2015aa}, \texttt{PyYAML}~\cite{Simonov:2014aa}, and \texttt{Adept}~\cite{adept-v11}.
In addition, interpreting Cantera-format mechanisms~\cite{Goodwin:2015aa} requires that module, although Chemkin-format mechanisms do not require any additional components.
...
Table~\ref{T:mechanisms} summarizes the chemical kinetics models used as benchmarks in this work, including the \ce{H2}\slash \ce{CO} model of Burke et al.~\cite{Burke:2011fh}, GRI-Mech 3.0~\cite{smith_gri-mech_30}, USC Mech version II~\cite{Wang:2007}, the isopentanol model of Sarathy et al.~\cite{Sarathy:2013jr}, and version 3.1 of the LLNL \emph{n}-heptane model~\cite{Mehl:2009aa,Mehl:2011cn}.
...
Sarathy isopentanol & 360 & 2172 & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & \cite{Sarathy:2013jr} \\
LLNL \emph{n}-heptane & 654 & 5258 & $\times$ & $\times$ & & & & \cite{Mehl:2009aa,Mehl:2011cn} \\
\bottomrule
\end{tabular}
\caption{
Summary of chemical kinetic models used as benchmark test cases. All mechanisms contain third-body reactions with enhanced third bodies. The ``Lind.'', ``Troe'', ``SRI'', ``P-log'', and ``Cheb.'' columns indicate the presence of Lindemann, Troe, and SRI falloff; logarithmic pressure interpolation; and Chebyshev pressure-dependent reactions, respectively. Two reactions with photons were removed from the Sarathy et al.~\cite{Sarathy:2013jr} model since neither Cantera nor \texttt{pyJac} support these.
}
\label{T:mechanisms}
\end{table}
...
\toprule
Parameter & \ce{H2} & \ce{CH4} & \ce{C2H4} & \ce{iC5H11OH} & \ce{nC7H16} \\
\midrule
$\phi$ & \multicolumn{5}{c}{1} \\
$T_{\text{in}}$ & \multicolumn{5}{c}{\SIlist{400;600;800}{\kelvin}} \\
$p$ & \multicolumn{5}{c}{\SIlist{1;10;25}{\atm}} \\
$N_{\text{p}}$ & \multicolumn{5}{c}{100} \\
$\tau_{\text{res}}$ & \SI{10}{\milli\second} & \SI{5}{\milli\second} & \SI{100}{\micro\second} & \SI{100}{\micro\second} & \SI{100}{\micro\second} \\
$\tau_{\text{mix}}$ & \SI{1}{\milli\second} & \SI{1}{\milli\second} & \SI{10}{\micro\second} & \SI{10}{\micro\second} & \SI{10}{\micro\second} \\
$\tau_{\text{pair}}$ & \SI{1}{\milli\second} & \SI{1}{\milli\second} & \SI{10}{\micro\second} & \SI{10}{\micro\second} & \SI{10}{\micro\second} \\
$N_{\text{res}}$ & 10 & 10 & 10 & 5 & 5 \\
\bottomrule
\end{tabular}
\caption{
PaSR parameters used for hydrogen\slash air, methane\slash air, ethylene\slash air, isopentanol\slash air, and \emph{n}-heptane\slash air premixed combustion cases, where $\phi$ indicates equivalence ratio, $T_{\text{in}}$ is the temperature of the inflowing particles, $p$ is the pressure, $N_{\text{p}}$ is the number of particles, $\tau_{\text{res}}$ is the residence time, $\tau_{\text{mix}}$ is the mixing time, $\tau_{\text{pair}}$ is the pairing time. and $N_{\text{res}}$ is the number of residence times simulated.
}
...
As a result of the aforementioned difficulties, we used the \texttt{Adept} software library~\cite{hogan2014fast,adept-v11} to obtain accurate Jacobian matrices, via automatic differentiation using expression templates.
In our experiences, even highly accurate numerical Jacobians obtained using multiple-term Richardson extrapolants~\cite{Richardson:1911cl,Lyness:1966aa,Lyness:1969bt} exhibited non-negligible errors for some conditions.
...
For all test cases, the correctness of the species and reaction rate subroutines were determined through comparison with Cantera~\cite{Goodwin:2015aa}; however, due to issues with nonphysical (i.e., negative) mass fraction values that arise during the finite differencing, the numerical Jacobian matrices were not calculated using Cantera but rather the generated derivative subroutines directly.
...
However, while many quantities calculated via \texttt{TChem} agreed closely with those from both Cantera and \texttt{pyJac}, we observed discrepancies in the species net production rates that correlated with errors in the derivative source term and Jacobian matrix.
...
As a result, we will not present detailed Jacobian matrix comparisons between \texttt{TChem} and \texttt{pyJac} here.
