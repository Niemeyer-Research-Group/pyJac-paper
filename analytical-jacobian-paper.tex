...
Furthermore, during our testing it became apparent that \texttt{TChem} is not thread-safe
...
Table~\ref{t:cpu_comp} summarizes the performance ratio between Jacobian matrices calculated using \texttt{pyJac} and finite differences\slash \texttt{TChem}; \texttt{pyJac}-based routines perform approximately \SIrange{1.1}{2.2}{$\times$} faster than \texttt{TChem} on a single-threaded basis, depending on the size of the model.
...
\begin{figure}[tbp]
    ...
    at the values associated with each model given in Table~\ref{T:error}.}
    \label{F:cpu_scaling}
\end{figure}

Lastly, Figure~\ref{F:cpu_scaling} shows the scaling of performance with the
number of cores, ranging from 1 to 32, and the number of conditions fixed at
the values given for each model in Table~\ref{T:error}.
...
increasing number of processors. We hypothesize that the reasons for lower
scaling efficiency of these two models actually differ. The cost of evaluation
\ce{H2}\slash \ce{CO} is low enough, particularly at increasing numbers of CPU
...
The \texttt{pyJac} CPU-based Jacobian evaluation performs \numrange{3}{7.5} times faster than a forward-finite-difference approach, while a more modest improvement speedup of \SIrange{1.07}{2.16}{$\times$} was found compared to the existing \texttt{TChem} software on a single-threaded basis.
Again it is noted that \texttt{TChem} is \textbf{not} thread-safe when parallelized with OpenMP, while \texttt{pyJac} is easily parallelized with the same approach; for multicore CPUs \texttt{pyJac} is expected to greatly outperform \texttt{TChem}.
...
Jacobian matrices, the results differ compared with those obtained from evaluating the same matrices
in serial.
...
discussed elsewhere in this paper. In short, the testing program first uses \texttt{TChem} to
...
Next, this process is repeated, but with OpenMP parallelization of the outer loop enabled.
