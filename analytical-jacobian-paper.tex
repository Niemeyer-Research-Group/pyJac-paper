...
This work developed the theory behind an analytic Jacobian matrix evaluation approach for constant-volume chemical kinetics, including new derivations of partial derivatives with respect to modern reaction pressure-dependence formulations.
...
The presented methodology was implemented in the open-source software package \texttt{pyJac}, which generates custom source code files for evaluating chemical kinetics Jacobian matrices on both CPU and GPU systems.
...
Finally, further study will be performed into the benefits of the current hard-coded, compiled Jacobian evaluation subroutine approach and a loop\slash branch-based approach, in order to determine how to obtain the best performance scaling with kinetic model size.
