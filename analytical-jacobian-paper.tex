...
However, power consumption and heat dissipation issues disrupted this trend recently, slowing the pace of increases in processor speed.
...
Note that we assume a constant-pressure system in the current approach.\todo{Need to say more?}
...
As with the update scheme used in Eq.~\eqref{e:jac_temperature_update}, those in Eqs.~\eqref{e:pindep_jac_species},~\eqref{e:thd_jac_species}~and~\eqref{e:pdep_jac_species} significantly reduce the computational overhead of Jacobian evaluation.
...
As a practical note, the size of the generated Jacobian subroutines are such that even relatively modern compilers, e.g., \texttt{gcc} 4.4.7 and the CUDA compiler \texttt{nvcc} 7.0 can crash when compiling the generated Jacobian for even modestly sized kinetic models (e.g., GRI~\cite{smith_gri-mech_30} for \texttt{nvcc} and USC Mech II~\cite{Wang:2007} for \texttt{gcc}).
