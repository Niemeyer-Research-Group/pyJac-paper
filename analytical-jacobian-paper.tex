...
In general, the GPU Jacobian evaluated large numbers of conditions much faster than the CPU version, and thus the mean and standard deviation were calculated from the runtime of one hundred individual runs.
...
\bottomrule
\end{tabular}
\caption{Ratio of Jacobian matrix evaluation runtimes for \texttt{TChem} and \texttt{pyJac}.}
\label{t:cpu_comp}
\end{table}
...
Figure~\ref{F:gpu_norm} shows the longest evaluation time---normalized by the number of conditions---for each mechanism plotted against the number of reactions in the mechanism.
As in Fig.~\ref{F:cpu_perf}, we observed a slight superlinear scaling of the performance with number of reactions.
