...
and, 
\begin{equation}
T_{\partial Y, i, \alpha} = \hat{\alpha}_{i}R_i
\end{equation}
where $\hat{\alpha}_{i}$ is the median third-body efficiency for reaction $i$ (typically unity).
This choice will result in the least additional correction term additions needed for individual species; corrections for individual species are unavoidable however when $\alpha_{i \numsp}$ is non-zero, due to the division by a different molecular weight.
...
\mathcal{J}_{k+1,j+1} &\pluseq&
  ...
  \Biggl[
    ...
    T_{\partial Y, i, \alpha} +  
    	\left(\alpha_{ij} - \hat{\alpha}_{i} - \alpha_{i \numsp}\frac{W_j}{W_{\numsp}}\right)R_i
    \nonumber \\
&& +\:  c_i 
...
and, 
\begin{equation}
T_{\partial Y, i, \alpha_{f}} = \delta_{\text{M}}
								\left(T_{P_{r,i}} + T_{F_i}\right)
								\hat{\alpha}_{i}
								R_i
\end{equation}
...
&& +\:  \left(T_{P_{r,i}} + T_{F_i}\right)
		\left(
			\delta_{M}\left(\alpha_{ij} - \hat{\alpha}_i - \alpha_{i \numsp}\frac{W_j}{W_{\numsp}}\right) \right. \nonumber \\
&& +\:			
...
The Python package \texttt{pyJac} implements the methodology for producing analytical Jacobian matrices described in the previous sections, which we released openly online~\cite{Niemeyer:2015im} under a modified BSD license.
