...
&& -\: k_{r,i} \left( \frac{1}{T} \left( \beta_i + \frac{T_{a,i}}{T} \right) - \sum_{j=1}^{N_{\text{sp}}} \nu_{ji} \dydx{B_j}{T} \right) \prod_{j=1}^{N_{\text{sp}}} [X_j]^{\nu_{ji}^{\prime\prime}} + \frac{R_{r,i}}{T} \prod_{j=1}^{N_{\text{sp}}} \nu_{ji}^{\prime\prime} \nonumber \\
&=& \frac{R_{f,i}}{T} \left( \beta_i + \frac{T_{a,i}}{T} - \sum_{j=1}^{N_{\text{sp}}} \nu_{ji}^{\prime} \right) \nonumber \\
&& -\: R_{r,i} \left[ \frac{1}{T} \left( \beta_i + \frac{T_{a,i}}{T} - \prod_{j=1}^{N_{\text{sp}}} \nu_{ji}^{\prime\prime} \right) - \prod_{j=1}^{N_{\text{sp}}} \nu_{ji} \dydx{B_j}{T} \right] \IEEEeqnarraynumspace
\end{IEEEeqnarray}}%
...
\end{equation}
Next, instead of summing over all reactions for a single species, we transform the Algorithm to compute the temporary product for a single reaction $i$ and add to all relevant species:
...
\mathcal{J}_{k+1,j+1} \pluseq
	\frac{W_k}{W_j}
	\Biggl(
		T_{dy_,i,\text{independent}} + \frac{\dot{\omega}_kW}{\rho} \\
	+ \left(
...
\begin{equation}
	T_{\partial Y, i, \text{third-body}} = -W\left[X\right]_i + \rho\hat{\alpha}_{ij}
\end{equation}
where $\hat{\alpha}_{ij}$ is defined as the median non-unity third-body efficiency for reaction $i$.
...
\mathcal{J}_{k+1,j+1} \pluseq
	\frac{W_k}{W_j}
	\Biggl(
		T_{\partial Y_,i,\text{independent}} + T_{\partial Y, i, \text{third-body}} + \frac{\dot{\omega}_kW}{\rho} + 
	\rho\left(\alpha_{ij} - \hat{\alpha}_{ij}\right) \\
	+ \left(
		\nu_{ji}^{\prime}k_{f,i}\left[X_j\right]^{\nu_{ji}^{\prime}-1}
	\right)
	\prod_{\substack{n=1 \\ n \neq j}}^{N_{\text{sp}}}[X_n]^{\nu_{ni}^{\prime}} +
...
\begin{equation}
T_{\partial Y, i,\text{falloff}} = c_i\left(T_{P_{r,i}} + T_{F_i}\right)\left(-W+\frac{\rho}{\left[X\right]_i}\hat{\alpha}_{ij}\right)
\end{equation}
...
\mathcal{J}_{k+1,j+1} \pluseq
	\frac{W_k}{W_j}
	\Biggl(
		T_{\partial Y_,i,\text{independent}} + T_{\partial Y, i, \text{falloff}} + \frac{\dot{\omega}_kW}{\rho} + \\
	c_i\left(T_{P_{r,i}} + T_{F_i}\right)\left(\frac{\rho}{\left[X\right]_i}\delta_{M}\left(\alpha_{ij} - \hat{\alpha}_{ij}\right) + \frac{\left(1 - \delta_{\text{M}}\right)}{Y_m}\right) + \nonumber\\
	\left(
		\nu_{ji}^{\prime}k_{f,i}\left[X_j\right]^{\nu_{ji}^{\prime}-1}
	\right)
	\prod_{\substack{n=1 \\ n \neq j}}^{N_{\text{sp}}}[X_n]^{\nu_{ni}^{\prime}} +
...
Like the update scheme used in Eq.~\eqref{e:}
