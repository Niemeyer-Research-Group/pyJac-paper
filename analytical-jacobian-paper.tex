...
However, the chemical kinetic models for hydrocarbon fuels of practical interest typically have large numbers of species and reactions and exhibit high levels of mathematical stiffness in the governing differential equations, particularly for larger molecules.
...
First, we establish the correctness of the Jacobian matrices for kinetic models of hydrogen, methane, ethylene, and isopentanol oxidation (number of species ranging \numrange{13}{360}) by showing agreement within \SI{1e-3}{\percent} of matrices obtained via automatic differentiation.
...
{\em Number of processors used:} one or more if present \todo{multiple cores only used for testing, see below note} \\
...
{\em Additional comments:} Current version and support available at \url{https://github.com/kyleniemeyer/pyjac/}\\
  %Provide any additional comments here.
   \\
{\em Running time:} Problem dependent; from a few seconds to an hour depending on the model size. \todo{does this describe the generation of the subroutines? or functional/performance testing?.  If just the generation/compilation, it is likely much less than an hour (~15 min max with parallel compilation}\\
...
In the current implementation of \texttt{pyJac}, we assume a constant-pressure system\footnote{In the context of multidimensional reactive flows, the constant-pressure assumption signifies that pressure remains constant during the reaction substep of an operator-splitting scheme (and is thus an input variable to the kinetics problem), not that the pressure is fixed throughout the entire simulation. This assumption is commonly used in low-speed combustion simulations and equivalent to that used in standard chemical-kinetics species production rate evaluation routines such as \texttt{CKWYP} in Chemkin~\cite{chemkin:2012}.}; thus, we use Eq.~\eqref{e:state} to determine density rather than including it in the differential system given by Eq.~\eqref{e:vars}.
...
Unfortunately, simulations of low-speed open combustion systems typically rely on the constant-pressure assumption~\cite{Law:2006tu}, with the constant volume assumption applicable to enclosed systems (e.g., internal combustion engine cylinder).
...
This is a planned future improvement for \texttt{pyJac}.
...
\end{IEEEeqnarray}%
In doing so, the temporary product $T_{\partial T,  i}$ must only be evaluated once for each reaction, rather than once for every species in each reaction with a non-zero net production or consumption rate.
...
\begin{equation}
  \delta_M =
\begin{dcases}
1 &\text{if the mixture acts as \nth{3} body, or} \\
0 &\text{if species $m$ acts as \nth{3} body.}
\end{dcases}
\end{equation}
