...
\texttt{pyJac} requires the Python modules \texttt{NumPy}~\cite{Walt:2011aa}; optional dependencies include \texttt{bitarray}~\cite{bitarray:2015} for the cache optimizer, and the performance testing module additionally requires \texttt{Cython}~\cite{Behnel:2011aa}, Cantera~\cite{Goodwin:2014aa}, \texttt{PyYAML}~\cite{Simonov:2014aa}, and \texttt{Adept}~\cite{adept-v11}.
In addition, interpreting Cantera-format mechanisms~\cite{Goodwin:2014aa} requires that module, although Chemkin-format mechanisms do not require any additional components.
...
USC Mech II		& \num{900900}	& \SI{2.301e-5}{\percent} & \SI{1.575e-2}{\percent} \\
\ce{iC5H11OH}	& \num{450900}	& 	& \\
LLNL \ce{nC7H16}& \num{450900}	&	& \\
\bottomrule
\end{tabular}
