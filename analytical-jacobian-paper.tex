...
\end{IEEEeqnarray}
for $k, j = 1, \ldots, \numsp - 1$, and the placeholder variable $\hat{\mathcal{J}}_{\numsp + 1, 1}$ may be updated using Eq.~\eqref{e:thd_jac_species}.
...
\end{dcases}
\end{equation}
Again, we note that the placeholder variable $\hat{\mathcal{J}}_{\numsp + 1, 1}$ may be updated as in Eq.~\eqref{e:pdep_jac_species}

Finally, all Jacobian entries $\mathcal{J}_{k+1,j+1}$, $\hat{\mathcal{J}}_{\numsp + 1, 1}$ must be finished with the addition of the species rate term:
{\allowdisplaybreaks \begin{IEEEeqnarray}{rCl}
