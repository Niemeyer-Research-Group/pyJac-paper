...
&& +\:  c_i\left(T_{P_{r,i}} + T_{F_i}\right)
		\left(
			\frac{\delta_{M}}{\left[X\right]_i}\left(\alpha_{ij} - \alpha_{i \numsp}\frac{W_j}{W_{\numsp}}\right) \right. \nonumber \\
&& +\:			
...
\texttt{pyJac} requires the Python modules \texttt{NumPy}~\cite{Walt:2011aa}; optional dependencies include \texttt{Numberjack}~\cite{Hebrard:2010aa} with \texttt{SCIP}~\cite{Achterberg:2009aa} for the cache optimizer, and the performance testing module additionally requires \texttt{Cython}~\cite{Behnel:2011aa}, Cantera~\cite{Goodwin:2014aa}, \texttt{PyYAML}~\cite{Simonov:2014aa}, and \texttt{numdifftools}~\cite{Brodtkorb:2015aa}.
...
\midrule
\ce{H2}\slash \ce{CO} & \num{900900} & \SI{1.028e-9}{\percent} & \SI{3.257e-8}{\percent} \\
% unthresholded values: \SI{5.031e-6}{\percent} & \SI{0.866}{\percent}
GRI-Mech 3.0		& \num{450900}	& \SI{3.671e-3}{\percent} & \SI{4.145e-1}{\percent} \\
