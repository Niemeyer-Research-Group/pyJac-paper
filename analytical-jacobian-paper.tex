...
Reported mean evaluation times were computed from the wall-clock run times of ten individual calculations.\todo{was number of cores\slash threads taken into account?}{}
...
These fits suggest that all the available codes scale superlinearly---nearly quadratically---with number of reactions.\todo{looks like TChem scales sublinearly or nearly linearly, no?}{}
...
Figure~\ref{F:gpu_norm} shows the longest \texttt{pyJac} evaluation time---normalized by the number of conditions---for each kinetic model plotted against the number of reactions, and compared with the performance of the GPU-based finite difference technique.\todo{what about number of conditions for the FD? was this done at the same \# as pyJac?}{}
...
The least-squares best-fit lines for the \texttt{pyJac} and finite-difference results---with $R^2$ values of 0.95 and 0.77, respectively---are of similar polynomial orders as those of the CPU-based Jacobian evaluation, suggesting dependence on the methods rather than hardware.
...
Thus, for larger models, it will be more efficient to use an analytical Jacobian formulation on the GPU than a finite difference formulation.
A sparse Jacobian formulation would further benefit GPU evaluation due to the greatly reduced memory traffic requirements (i.e., reduced number of global reads and writes).
...
For example, the CPU Jacobian evaluation subroutine for USC-Mech II comprises over \num{360000} total lines spread across 22 files (not including the supporting files for, e.g., species and reaction rate evaluations)---attempting to incorporate the entire matrix evaluation in a single file resulted in the \texttt{gcc} 4.4.7 compiler crashing.\todo{are these numbers up to date?}{}
