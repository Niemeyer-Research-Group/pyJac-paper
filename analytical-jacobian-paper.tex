...
Further, existing analytical Jacobian tools do not optimize evaluations or support the emerging SIMD processors such as GPUs.
...
As has been observed by others, in recent years the sizes of chemical kinetic models for hydrocarbon fuels

In the past, regular computational power increases could be counted on to enable previously inaccessible simulations.
This trend has been disrupted recently due to power consumption and heat dissipation issues, slowing the pace of increases in processor speed.
...
Recognizing that the SIMD parallelism model fits well with the operator-split chemistry integration, which forms the basis of most reactive-flow codes~\cite{Oran:2001aa}, a number of studies in recent years~\cite{niemeyer_accelerating_2014,niemeyer_turbulence-chemistry_2011,shi_accelerating_2012,shi_redesigning_2011,spafford_accelerating_2010,stone_techniques_2013} have explored the use of SIMD processors to accelerate reactive-flow codes.
...
To this end, \texttt{pyJac} is capable of generating subroutines for species and reaction rates, derivative source terms, and analytical chemical Jacobian matrices for the CUDA programming language~\cite{Nickolls:2008:SPP:1365490.1365500}, a widely used language for NVIDIA GPUs.
...
rationale: computational cost of a forward finite difference approximation scales with the square of the number of species, while cost of evaluating an analytical Jacobian grows linearly with the number of species~\cite{Lu:2009gh}.

% past work
%Adifor~\cite{Bischof:1996cc}, automatic differentiation
Over the past few years, a number of research groups developed analytical Jacobian generators for chemical kinetics, although as will be discussed the software package introduced here offers a number of improvements.
...
Recently, Dijkmans et al.~\cite{Dijkmans:2014bb} used a GPU-based analytical Jacobian combined with tabulation of temperature-dependent functions based on polynomial interpolations to accelerate the integration of chemical kinetics ODEs, similar to the earlier approach of Shi et al.~\cite{shi_redesigning_2011}.
...
Furthermore, to the authors' knowledge, unlike \texttt{pyJac}, none of the previously demonstrated analytical Jacobian generation implementations support the newer pressure-dependent reaction formulations (i.e., logarithmic interpolation and Chebyshev polynomial).
