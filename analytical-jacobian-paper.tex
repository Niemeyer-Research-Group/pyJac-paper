...
We also initially made comparisons with the \texttt{TChem} package~\cite{Safta:2011vn,TChem:v0.2} for validation purposes, using the \ce{H2}\slash \ce{CO} model.
...
During testing, it became apparent that \texttt{TChem} is not thread-safe when parallelized with OpenMP.
This is discussed further in \ref{A:tchem}.
Therefore, all CPU performance comparison between \texttt{pyJac}, \texttt{TChem} and the finite-difference method was carried in a single-threaded manner.
