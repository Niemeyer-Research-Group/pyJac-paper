...
In order to integrate the stiff equations governing chemical kinetics, generally implicit algorithms are used that require frequent Jacobian matrix evaluations; in addition, various computational combustion diagnostics methods rely on accurate Jacobian matrices.
Typically, Jacobian matrices are calculated numerically using finite differences, but this poses significant computational demands for larger chemical kinetic models since the number of chemical source term evaluations scales with the square of species count.
Further, existing analytical Jacobian tools do not optimize evaluations or support emerging SIMD processors such as GPUs.
...
First, we establish the correctness of the resulting Jacobian matrices through comparison with those obtained from high-order finite differences using rates from Cantera.
We then demonstrate the improvement in performance, via matrix evaluation timing comparisons, achievable on CPUs and GPUs by optimizing the order of matrix element calculations.
%Finally, we explore the sparsity and characteristics of Jacobian matrices for a wide range of chemical kinetic models describing the oxidation of various hydrocarbon fuels.
The Jacobian matrix generator we describe here will be particularly useful for reducing the cost of integrating chemical source terms with implicit algorithms in particular and algorithms that require an accurate Jacobian matrix in general.
...
\todo{Are we doing sparsity? We mention it in the abstract. (Removed from abstract for now.)}
...
As the need for detailed and accurate chemical kinetics models in predictive reactive-flow simulations has become recognized in recent years, simultaneously such models describing the oxidation of hydrocarbon fuels grew significantly in size and complexity.
...
Since in general the computational cost of solving the associated, generally stiff, systems of equations scales at best quadratically with the number of species---and at worst, cubically---models of this size pose challenges even for lower-dimensional analyses, and cannot practically be used directly in multidimensional reactive-flow simulations.
...
Such methods can be combined in multiple stages for significant a priori reduction~\cite{Lu:2008bi,Niemeyer:2014,Niemeyer:2015wq}, or applied dynamically during a simulation to achieve greater local savings~\cite{Banerjee:2006,Liang:2009,Shi:2010,Gou:2013eu,Yang:2013ip,Curtis:2015aa}.
...
However, by using an analytical formulation for the Jacobian matrix rather than a typical finite difference approximation, the cost of the numerous evaluations can be reduced from growing with the square of the number of species to a linear dependence~\cite{Lu:2009gh}.

In parallel to the potential improvements in stiff, implicit integrators used for chemical kinetics, another route to improving performance.\todo{Not sure what we're saying here}
In the past, regular computational power increases could be counted on to enable previously inaccessible simulations (i.e., Moore's Law).
However, this trend has been disrupted recently due to power consumption and heat dissipation issues, slowing the pace of increases in processor speed.
While the central processing unit (CPU) market has seen performance gains via multi-core parallelism, single-instruction multiple data processors (SIMD), e.g., graphics processing units (GPUs), recently emerged as a low-cost, low-power, and massively parallel high-performance computing alternative. 
GPUs---originally developed for graphics\slash video processing and display purposes---consist of many hundreds to thousands of cores compared to tens of cores found on a typical CPU.
Recognizing that the SIMD parallelism model fits well with the operator-split chemistry integration, which forms the basis of most reactive-flow codes~\cite{Oran:2001aa}, a number of studies in recent years~\cite{Spafford:2010aa,Shi:2011aa,Niemeyer:2011aa,Shi:2012aa,Stone:2013aa,Niemeyer:2014aa} explored the use of SIMD processors to accelerate the integration of chemical kinetics in reactive-flow codes.
While explicit methods offer significant improvements in performance for nonstiff and moderately stiff chemical kinetics~\cite{Niemeyer:2014aa}, experiences thus far suggest that implicit or similar algorithms may still be required to handle stiff chemistry.
...
Thus, motivated by the potential cost reductions offered by analytical Jacobian matrix formulations, over the past few years a number of research groups developed analytical Jacobian generators for chemical kinetics, although as will be discussed the software package introduced here offers a number of improvements.
The TChem toolkit developed by Safta et al.~\cite{Safta:2011vn} was one of the first software packages developed for calculating the analytical Jacobian matrix, but provides this functionality through an interface rather than generating customized source code for each mechanism.
...
As will be discussed in greater detail, unlike the current work, the approach of Dijkmans et al.~\cite{Dijkmans:2014bb} used the GPU to calculate the elements of a single Jacobian matrix in parallel, rather than many different matrices corresponding to different states.
...
In addition, since the programming paradigm to achieve efficient code evaluation for SIMD processors is quite different than that of normal CPUs, optimization strategies specifically targeted at accelerating Jacobian evaluation on GPUs will be laid out and tested.
...
Note that we assume a constant-pressure system.\todo{Need to say more?}
...
\begin{equation}
c_i = \begin{dcases}
  1 &\text{elementary reaction,} \\
  [X]_i &\text{\nth{3}-body enhanced reaction,} \\
  \frac{P_{r i}}{1 + P_{r i}} F_i &\text{unimolecular/recombination falloff reaction, and} \\
...
Further, even for relatively small kinetic mechanisms, e.g., GRI-Mech 3.0~\cite{smith_gri-mech_30}, the number of lines of code produced is quite substantial.
...
This section will lay out a restructuring of Algorithm~\eqref{p:naive_psuedo}, Algorithm~\eqref{p:updated_pseudo}, that greatly accelerates Jacobian evaluation via reuse of temporary products to reduce computational overhead.
...
For third body-enhanced reactions, we define a relatively simple temporary term:
\begin{equation}
...
For reactions with falloff dependence, we define a temporary term corresponding to the $P_r$ polynomial in Eqs.~\eqref{dCidY_falloff}~and~\eqref{dCidY_chem}:\todo{check nomenclature here}
\begin{equation}
  T_{P_{r, i}}  =
\begin{dcases}
  \hfil 0 \; & \text{if Lindemann,} \\
  \frac{1.0}{1.0 + P_r} \; & \text{if falloff reaction,} \\
  \frac{-P_r}{1.0 + P_r} \; & \text{if chemically activated reaction.}
\end{dcases} 
\end{equation}
...
This update scheme can often be simplified due to the choice of $\hat{\alpha}_i$, which minimizes the number of third-body efficiency correction terms needed for a reaction, and the zeroing of the species molar concentration product terms as discussed above.
...
First, a suitable seed reaction is selected, based on the reaction whose participating species (i.e., not third bodies) themselves participate in the largest number of total reactions.
The seed reaction is used to initialize the reordered reaction set.
...
As a practical note, the size of the generated Jacobian subroutines are such that even relatively modern compilers, e.g. \texttt{gcc} 4.4.7 and the CUDA compiler \texttt{nvcc} 7.0 can crash when compiling the generated Jacobian for even modestly sized kinetic mechanisms (e.g. GRI~\cite{smith_gri-mech_30} for \texttt{nvcc} and USC Mech II~\cite{Wang:2007} for \texttt{gcc}).
To avoid this limitation \texttt{pyJac} splits large Jacobians into several smaller subroutines when appropriate, which are compiled separately and called from the main Jacobian subroutine.
...
\begin{algorithmic}[0]
  ...
  \While{$\Set{\text{S}} \subseteq \Set{\text{R}}$}
...
\begin{algorithmic}[0]
  \State Denote the function $Species\left(R\right)$ as a function that returns the (non-third body) participating species of a reaction $R$
  \State Denote the ith reaction rate as $Rate_i$, and the jth net species production rate as $Spec_j$
  \For{Reaction $R_i \in \Set{Reactions}$}
  		\For{Species $S_j \in Species\left(R_i\right)$}
  			\State $Spec_j \pluseq \nu_{j, i} \times Rate_i$
  		\EndFor
  \EndFor
\end{algorithmic}
...
First, SIMD devices can execute all species rate updates for a reaction with with no chance of a stall due to data dependence as all the species rate updates are independent.
Secondly, the net reaction rate is used between multiple update steps and is thus highly likely to be found in the local L1 cache or in a register, cutting down global memory loads.
...
However, even use of a small number of shared memory cache spots can accelerate computation.
...
The methodology for producing analytical Jacobian matrices described in the previous sections was implemented in the Python package \texttt{pyJac}, available openly online~\cite{Niemeyer:2015im} under a modified BSD license. \todo{Maybe not for WSS paper, but get DOI for v1 release}
In order to demonstrate the correctness and computational performance of the generated analytical Jacobian matrices, multiple chemical kinetic models were chosen as test cases, selected to represent a wide spectrum of sizes, classes of fuel species, and types of reaction rate pressure dependence formulations (e.g., various falloff formulations). \todo{Add Chebyshev, pressure log when true}
...
For both the correctness and performance tests, thermochemical composition data were generated using stochastic partially stirred reactor (PaSR) simulations.
A detailed description of the PaSR methodology and implementation can be found in \ref{A:pasr}.
Nine premixed PaSR simulations were performed for each fuel\slash mechanism, with the parameters given in Table~\ref{T:pasr_parameters}.
...
In order to test the correctness of the Jacobian matrices produced by \texttt{pyJac}, the resulting analytical matrices were compared to numerical approximations based on finite differences.
However, while these are used commonly to provide numerical approximations to Jacobian matrices, potential scaling issues due to large disparities in species mass fractions and associated net production rates can cause challenges in selecting appropriate differencing step sizes~\cite{Shampine:1994aa}.
For example, when initially attempting to evaluate derivatives using sixth-order central differences with increments calculated similarly to that used by the implicit CVODE integrator in the SUNDIALS suite~\cite{Hindmarsh:2005hg}, we encountered large errors in various partial derivatives under certain conditions.
Adjusting the relative and absolute tolerances reduced some of these errors, but consistent values could not be found that sufficed over all states considered---in particular, extremely small tolerances were required for states near equilibrium, but these same tolerances caused larger errors at other states.
...
In reporting the discrepancy between the analytical and numerical Jacobian matrices, denoted $\mathcal{J}$ and $\hat{\mathcal{J}}$ respectively, the  the norm of the relative errors of matrix elements $E_\text{rel} = \norm{ ( \hat{\mathcal{J}}_{ij} - \mathcal{J}_{ij} ) / \hat{\mathcal{J}}_{ij} }_p$, where $p = 2$ (the Frobenius norm), was used to quantify error.
...
The latter indicates overall error in the matrix but can be dominated by large elements, so for the current purposes the former error measure can be more useful in identifying discrepancies in both larger and smaller matrix elements.

Table~\ref{T:error} presents the correctness testing results for each benchmark case.
For certain conditions, large relative errors were observed in extremely small nonzero elements (e.g., magnitude $\sim$\num{e-22}) likely due to roundoff errors; thus, error statistics were only based on matrix elements $\mathcal{J}_{ij}$ where $\abs{\mathcal{J}_{ij}} \geq \norm{\mathcal{J}}_p / \num{e20}$.
...
\midrule
\ce{H2}\slash \ce{CO} & \num{900900} & \SI{5.031e-6}{\percent} & \SI{0.8657}{\percent} \\
...
\label{s:performance}
In order to test the performance of \texttt{pyJac}, the PaSR data generated for validation of the various kinetic mechanisms was used.
The CPU Jacobian was compiled using \texttt{gcc} 4.4.7, and run on two six-core Xeon X5650 processors, parallelized using OpenMP.
In order to have a fair comparison between mechanisms, the mean simulation time over ten runs was normalized by the number of conditions in the generated PaSR data.
The CPU Jacobian was tested both with and without cache optimization, as described in Section~\eqref{s:cache_opt}, to assess it's impact on the performance of the CPU code.

The GPU Jacobian was tested on a single NVIDIA Tesla C2075, and was compiled using \texttt{nvcc} 7.0.
The performance data for the GPU is presented as the total runtime for the various optimization options, plotted against the number of conditions, in order to demonstrate that the GPU has reached full utilization.
Again, ten runs were performed for each number of conditions for averaging and standard deviation purposes.
The GPU Jacobian was tested with and without cache optimization and shared memory caching, as described in Section~\eqref{s:smem}, and all combinations thereof.
...
In addition, future versions of \texttt{pyJac} will include the option of assuming constant-volume combustion, and support generating Fortran and Matlab source code.
...
The discrete time step between inflow\slash outflow and pairing events and the sub-time step size separating mixing\slash reaction fractional steps, both held constant in the current implementation, are determined via 
\begin{align}
