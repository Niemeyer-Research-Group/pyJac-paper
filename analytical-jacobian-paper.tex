...
The CPU Jacobian was compiled using \texttt{gcc} 4.4.7, and run a single core of Xeon X5650 processor.
The mean and standard deviation evaluation times were computed from the runtime of ten individual runs. 
The GPU Jacobian was tested on a single NVIDIA Tesla C2075, and was compiled using \texttt{nvcc} 7.0.
The GPU Jacobian in general, was much faster to evaluate large numbers of conditions and therefore the mean and standard deviation were calculated from the runtime of one hundred individual runs.

The CPU performance of \texttt{pyJac} is compared to the commonly used \texttt{TChem}~\cite{safta2011tchem} code in Fig.~\ref{F:cpu_perf}.
For the \ce{H2}\slash \ce{CO} and GRI-Mech 3.0 mechanisms, \texttt{pyJac} offers a significant improvement (as summarized in Table~\ref{t:cpu_comp}) in evaluation speed over the \texttt{TChem} code.
However, for the USC mechanism the performance of the two codes is very similar.
It is also noted that the performance of both codes scale slightly above linearly with the number of reactions in the mechanism, as denoted by the dashed line in Fig.~\ref{F:cpu_perf}.
This result challenges the assumption that hard-coded and compiled subroutines will outperform a loop / branch based solution.
Of further note is the difficulties involved in the actual compilation of the Jacobian subroutine; for the USC mechanism the generated subroutine was so large that \texttt{gcc} 4.4.7 crashes during compilation.
As a result, the Jacobian evaulation must be split into several smaller subroutines, which each evaluate the contributions of a number of reactions.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.75\textwidth]{cpu_norm.pdf}
    \caption{Comparison of \texttt{pyJac} and \texttt{TChem} Jacobian evaluation speeds for the various kinetic mechanisms. 
...
\begin{tabular}{@{}l l l l@{}}
\toprule
Mechanism & $\frac{\bar{R}_{\texttt{TChem}}}{\bar{R}_{\texttt{pyJac}}}$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Mean Jacobian evaulation runtime comparison for \texttt{TChem} and \texttt{pyJac}}
\label{t:cpu_comp}
\end{table}

The GPU Jacobian evaluation performance of \texttt{pyJac} is investigated in Fig.~\ref{F:gpu_perf}.
In Fig.~\ref{F:gpu_mean}, the mean runtime for the GPU Jacobian evaluation is plotted against the number of conditions.
As the number of conditions increases the GPU becomes fully utilized and the growth rate of the runtime begins increasing linearly (here displayed on a semi-log plot).
In Fig.~\ref{F:gpu_norm}, the longest runtime (normalized by the number of conditions) for each mechanism is plotted against the number of reactions in the mechanism.
As in Fig.~\ref{F:cpu_perf} a slightly above linear scaling of the runtimes with number of reactions was observed.
...
\begin{figure}[tbp]
    ...
    \begin{subfigure}{0.5\textwidth}
        ...
        \label{F:gpu_norm}
    \end{subfigure}
    \caption{Performance of \texttt{pyJac} Jacobian evaluation for the GPU}
    \label{F:gpu_perf}
\end{figure}
...
Finally, a comparison between a hard-coded, compiled Jacobian evaluation subroutine and a loop/branch based Jacobian evaluation subroutine will be run to determine how to obtain the best performance scaling with mechanism size.
